...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const commonScrollTrigger = {
    ...
    about: {
      // 컨트롤러 등록(요소, 시작점, 끝나는점, 스크롤 동기화 여부 등)
      trigger: ".about", // 애니메이션 시작점과 끝나는 지점의 기준
      start: "top bottom", // 첫번째는 요소의 시작 위치, 두번째는 화면의 시작 위치
      scrub: 1.8, // 스크롤 동기화 여부, true일 경우 스크롤 타이밍에 맞춰 애니메이션 실행, 시간 적용 시 지정된 시간 만큼 지연 후 애니메이션 실행
      // markers: true, // 디버깅을 위한 마커 표시
    },
  ...
  // const titleSquare = document.querySelector(".tilte-square");
  ...
  function headerAnimation(xValue) {
    ...
    tl.to(
      ...
      {
        clipPath:
          "polygon(0 26%, 0 0, 35% 0%, 65% 0%, 100% 0, 100% 26%, 100% 51%, 100% 100%, 65% 100%, 35% 100%, 0 100%, 0 50%)",
